...
function Rating({ rating }: { rating: RatingType }) {
  const starts = Array.from({ length: rating.rate }, (_, index) => index + 1);
  return (
    <div>
      <span
        style={{
      ...
      >
        {calculateStars(rating.rate).starsFiled.map((start) => (
          <svg
            key={start}
        ...
        {calculateStars(rating.rate).starsNotFiled.map((start) => (
          <div key={start}>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
